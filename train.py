...
def compute_distillation_output_loss(p, t_p, model, loss):
    ...
    t_lcls, t_lbox, t_lobj = torch.zeros(1, device='cuda'), torch.zeros(1, device='cuda'), torch.zeros(1, device='cuda')
    ...
    if opt.prune:
        ...
        for m in model.modules():
            if isinstance(m, nn.Conv2d) and m not in excluded_layers:
                #pruning_plan = DG.get_pruning_plan( m, tp.prune_conv, idxs=strategy(m.weight, amount=0.08) )
                pruning_plan = DG.get_pruning_plan( m, tp.prune_conv, idxs=strategy(m.weight, amount=0.29) )
                #pruning_plan = DG.get_pruning_plan( m, tp.prune_conv, idxs=strategy(m.weight, amount=0.28) )
                #pruning_plan = DG.get_pruning_plan( m, tp.prune_conv, idxs=strategy(m.weight, amount=0.53) )
                print(pruning_plan)
...
def parse_opt(known=False):
    ...
    parser.add_argument('--prune', action='store_true', help='pruning')
